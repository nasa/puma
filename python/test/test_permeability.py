...
class TestPermeability(unittest.TestCase):
    ...
    def test_analytical_direct_noprec(self):
        ws = puma.generate_cylinder_square_array(100, 1. - 2. * np.pi * (0.1 ** 2.))
        ws.voxel_length = 1. / ws.matrix.shape[0]
        keff, _ = puma.compute_permeability(ws, (1, 1), solver_type='direct', precondition=False)
        np.testing.assert_array_almost_equal(keff, np.array([[2.71223274e-02, 0., 0.], [0., 2.71223274e-02, 0.], [0., 0., 5.48134246e-02]]))
    ...
    #     ws = puma.generate_cylinder_square_array(100, 1. - 2. * np.pi * (0.1 ** 2.))
    #     ws.voxel_length = 1. / ws.matrix.shape[0]
    #     keff, _ = puma.compute_permeability(ws, (1, 1), solver_type='bicgstab', tol=1e-7, maxiter=10000)
    #     np.testing.assert_array_almost_equal(keff, np.array([[2.71223274e-02, 0., 0.], [0., 2.71223274e-02, 0.], [0., 0., 5.48134246e-02]]), decimal=4)
    #
    # def test_analytical_bicgstab_nomf(self):
    #     ws = puma.generate_cylinder_square_array(100, 1. - 2. * np.pi * (0.1 ** 2.))
    #     ws.voxel_length = 1. / ws.matrix.shape[0]
    #     keff, _ = puma.compute_permeability(ws, (1, 1), solver_type='bicgstab', tol=1e-7, maxiter=10000, matrix_free=False)
    #     np.testing.assert_array_almost_equal(keff, np.array([[2.71223274e-02, 0., 0.], [0., 2.71223274e-02, 0.], [0., 0., 5.48134246e-02]]), decimal=4)
    #
    # def test_analytical_bicgstab_noprec(self):
    #     ws = puma.generate_cylinder_square_array(100, 1. - 2. * np.pi * (0.1 ** 2.))
    #     ws.voxel_length = 1. / ws.matrix.shape[0]
    #     keff, _ = puma.compute_permeability(ws, (1, 1), solver_type='bicgstab', tol=1e-7, maxiter=10000, preconditioner=False)
    #     np.testing.assert_array_almost_equal(keff, np.array([[2.71223274e-02, 0., 0.], [0., 2.71223274e-02, 0.], [0., 0., 5.48134246e-02]]), decimal=4)
    #
    # def test_analytical_bicgstab_nomf_noprec(self):
    #     ws = puma.generate_cylinder_square_array(100, 1. - 2. * np.pi * (0.1 ** 2.))
    #     ws.voxel_length = 1. / ws.matrix.shape[0]
    #     keff, _ = puma.compute_permeability(ws, (1, 1), solver_type='bicgstab', tol=1e-7, maxiter=10000, matrix_free=False, preconditioner=False)
    #     np.testing.assert_array_almost_equal(keff, np.array([[2.71223274e-02, 0., 0.], [0., 2.71223274e-02, 0.], [0., 0., 5.48134246e-02]]), decimal=4)

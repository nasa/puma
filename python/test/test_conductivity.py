...
class TestFiniteVolumeAnisotropicTC(unittest.TestCase):
    ...
    def test_artfib50_orient_prescribed_bc(self):
        ...
        # puma.plot_conductivity_fields(ws, T, q)
        np.testing.assert_array_almost_equal(keff, [0.06712707273637297, -0.0007017851098093986, 0.0017924686637479958], decimal=3)
        bc = puma.AnisotropicConductivityBC(ws)
        bc.yfaces[0] = 0
        bc.yfaces[1] = 1
        keff, T, q = puma.compute_thermal_conductivity(ws, cond_map, '', 's', tolerance=1e-5, prescribed_bc=bc)
        # puma.plot_conductivity_fields(ws, T, q)
        np.testing.assert_array_almost_equal(keff, [0.003760128942078807, 0.07922692884170093, 0.003888392944562686], decimal=3)
        bc = puma.AnisotropicConductivityBC(ws)
        bc.zfaces[0] = 0
        bc.zfaces[1] = 1
        keff, T, q = puma.compute_thermal_conductivity(ws, cond_map, '', 's', tolerance=1e-5, prescribed_bc=bc)
        # puma.plot_conductivity_fields(ws, T, q)
        np.testing.assert_array_almost_equal(keff, [0.0015291100308031456, -0.0016663003168615753, 0.044952464988095735], decimal=3)

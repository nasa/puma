...
typedef struct
{
  real_mc  x,  y,  z ;  /**< Vertex coordinates */
  real_mc nx, ny, nz ;  /**< Vertex normal */
} Vertex ;
...
//  inline void setWorkspace( puma::Matrix<float> *workspace ) { this->workspace = workspace; }
...
//  inline void set_ext_data  ( real_mc *data )
...
//  inline void set_int_data  () { _ext_data = false ;  _data = NULL ; }
   ...
   */
  inline const real_mc get_data  ( const int i, const int j, const int k ) const { return workspace->at(i,j,k);   }
   ...
   * \param k height of the cube
   */
//  inline void  set_data  ( const real_mc val, const int i, const int j, const int k ) { _data[ i + j*_size_x + k*_size_x*_size_y] = val ; }
...
protected :
  ...
  real_mc     *_data       ;  /**< implicit function values sampled on the grid */
  ...
  real_mc      _cube[8]{}    ;  /**< values of the implicit function on the active cube */

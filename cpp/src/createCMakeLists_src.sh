#!/bin/bash
## Change src/CMakeList.txt with the new header and source files.
## Usage: ./createCMakeLists_src.sh
...
do
     dir=`echo "$f" | awk -F"/" '{print $1 "/" $2}'` # first two folders
     if [ $f == "src/main.cpp" ] || [ $f == "src/pumamodule.cpp" ]; then
	 continue
     fi
     if [ $i -eq 0 ]; then
         list_cpp=$f

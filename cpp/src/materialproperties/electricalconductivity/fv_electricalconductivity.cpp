...
puma::Vec3<double> puma::compute_FVElectricalConductivity(Workspace *grayWS, puma::Matrix<double> *T, const std::map<int, double>& matCond,
    ...
    for (auto & it : matCond) {
        if (it.first < -0.5 || it.first > 32767) {
            segWS.printer->error("Finite Volume Electrical Conductivity Error: Invalid MatCond puma::Cutoff");
    ...
    FV_ElectricalConductivity cond(&segWS,T,condPairs,std::move(sideBC),std::move(solverType),dir,solverTol,solverMaxIt, print,numThreads);
    return cond.compute();
}


FV_ElectricalConductivity::FV_ElectricalConductivity(puma::Workspace *segWS, puma::Matrix<double> *T, std::map<int, double> matCond,
                                               std::string sideBC, std::string solverType, char dir, double solverTol,
                                               int solverMaxIt, bool print, int numThreads) {
    this->segWS = segWS;
    this->T = T;
...
puma::Vec3<double> FV_ElectricalConductivity::compute() {
    logInput();
    std::string errorMessage;
    if( !errorCheck(&errorMessage) ) {
        std::cout << "Electrical Conductivity Error: " <<  errorMessage << std::endl;
        return puma::Vec3<double>(-1,-1,-1);
    }
...
bool FV_ElectricalConductivity::logInput() {
...
bool FV_ElectricalConductivity::logOutput() {
    puma::Logger *logger = segWS->log;
...
bool FV_ElectricalConductivity::errorCheck(std::string *errorMessage) {

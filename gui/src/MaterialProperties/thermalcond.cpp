...
ThermalCond::ThermalCond(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ThermalCond){
    ui->setupUi(this);
...
void ThermalCond::setViewer(Viewer *view) {
    this->view = view;
...
void ThermalCond::on_comboBox_currentIndexChanged(int index)
{
    if(index == 0){
        ui->comboBox_2->setVisible(false);
        ui->bcSideDirectLabel->setVisible(false);
...
void ThermalCond::Calculate() {
    ...
    if (ui->comboBox->currentIndex() == 0){
        ...
        if(ui->xDirCheckBox->isChecked() == true){
            ...
            kx = puma::compute_EJThermalConductivity(workspace, &Tx,matCond,'x',accuracy,iterations,true,numThreads);
        }
        ...
        if (ui->yDirCheckBox->isChecked() == true){
            ...
            ky = puma::compute_EJThermalConductivity(workspace, &Ty,matCond,'y',accuracy,iterations,true,numThreads);
        }
        ...
        if (ui->zDirCheckBox->isChecked() == true){
            ...
            kz = puma::compute_EJThermalConductivity(workspace, &Tz,matCond,'z',accuracy,iterations,true,numThreads);
        }
    ...
    if (ui->comboBox->currentIndex() == 1){
        ...
        if(ui->xDirCheckBox->isChecked() == true){
            std::cout << "X-Direction:" <<  std::endl;
            kx = puma::compute_FVThermalConductivity(workspace, &Tx,matCond,bcSideDirection,solverType,'x',accuracy,iterations,true, numThreads);
        }

        std::cout << solverType << " " << bcSideDirection << std::endl;
        ...
        if (ui->yDirCheckBox->isChecked() == true){
            std::cout << "\nY-Direction:" <<  std::endl;
            ky = puma::compute_FVThermalConductivity(workspace, &Ty,matCond,bcSideDirection,solverType,'y',accuracy,iterations,true,numThreads);
        }
        ...
        if (ui->zDirCheckBox->isChecked() == true){
            std::cout << "\nZ-Direction:" <<  std::endl;
            kz = puma::compute_FVThermalConductivity(workspace, &Tz,matCond,bcSideDirection,solverType,'z',accuracy,iterations,true,numThreads);
        }
...
void ThermalCond::on_CalculateButton_clicked()
{
...
void ThermalCond::on_vtkButton_clicked()
{
...
void ThermalCond::on_binaryButton_clicked()
{
    if(Tx.size() == 0 && Ty.size() == 0 && Tz.size() == 0) {
        return;
    }

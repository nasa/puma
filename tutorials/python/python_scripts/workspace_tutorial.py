...
# In this tutorial we demonstrate how to create a workspace and perform basic operations on it,
# including cropping, rotation, thresholding. A workspace is the datastructure at the basis of both
# PuMA and pumapy and it is basically a container for the material sample that is being analyzed.
# A workspace is made of little cubes, or voxels (i.e. 3D pixels), holding a value. This simple element
# definition (formally called Cartesian grid) allows for very fast operations. Inside a workspace object,
# two different arrays are defined: one called "matrix" and the other called "orientation". Both of
# these are nothing but a 3D Numpy array for the matrix (X,Y,Z dimensions of the domain) and a 4D Numpy
# array for the orientation (dimensions of X,Y,Z,3 for vectors throughout the domain).
...
# This is automatically enforced by using the class methods, but it is not when assigning them directly as in the 
# last example. Let's now import a tomography image directly into a workspace (PuMA comes with some example files 
# that can be imported using the path_to_example_file function as shown below): 
...
# This specific tiff stack is 8 bit, so the grayscale values will range from 0 to 255. PuMA can also import 16bit 
# images, which allow a much larger range from 0 to 65535 (i.e. 2^16). The voxel length (in meters) of the workspace 
# can either be set during import of a 3D tiff, or manually afterwards, as shown below: 
...
# up/down arrows to skip +/-100 slices (if this does not work automatically, try clicking on the plot first). 
# In addition, on the bottom of the plot, the (x,y) coordinates are shown along with the corresponding grayscale value. 
...
# However, it is important to not fall in the trap of referencing the same Numpy array.
# Here is an example of how you SHOULD NOT perform cropping:
...
# As you can see from the output, both the original Workspace and the newly created one share the same Numpy array
# for the matrix class variable (the second one is only a section of it, also called a Numpy view). This way,
# when one is changed, the other one will be changed as well. It is important to make a copy of a domain if the
# original workspace needs to be kept. Next, we show how we can rescale a domain by a factor or resize it to a
# specified size.
